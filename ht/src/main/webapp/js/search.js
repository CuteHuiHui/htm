...
if(name==null|| name==""){
	//分页显示所有歌曲
	searchAllSong(15,1)
	function searchAllSong(pageS,currP){
		...
		$.get("song/pagination",{"pageS":pageS,"currP":currP},function(data){
			...
			for(var i=0; i<data.rows.length;i++){
				...
				+'<p><label style="width:300px;">'+data.rows[i].soname+'</label>'
				...
				if(data.rows[i].singer==null){
					...
					songStr+='<div  class="list_play col-lg-2 col-md-2 hidden-sm hidden-xs">'+data.rows[i].singer.sgname+'</div>'
				}
				songStr+='<div  class="list_add col-lg-2 col-md-2 hidden-sm hidden-xs">'+data.rows[i].soduration+'</div></div>';
			}
			$(".S_box").html(songStr);
	...
	//分页显示所有歌手
	function searchAllSinger(pageS,currP){
		var sgname=location.href.split("=")[1];
		$.get("singer/pagination",function(data){
			var singerStr="";
			for(var i=0;i<data.rows.length;i++){
				singerStr+='<div class="content3_1_1"><div class="col-lg-3 col-md-3 col-sm-4 col-xs-6">'
					+'<div class="thumbnail pull-left"><a href="page/singer_more.jsp?sgid='+data.rows[i].sgid+'"> <img src="'+data.rows[i].sgpicPath+'" />'
					+'<div class="caption"><h3>'+data.rows[i].sgname+'</h3></div></a></div></div></div>';
			}
...
function paginationSong(totalP){
	...
	if(totalP>=5){
		...
		for (var j = 1; j <= 5; j++) {
			pStr +='<li><a href="javascript:void(0)" onclick="searchAllSong(15,'+(j+(count*5))+')">'+(j+count*5)+'</a></li>';
		}
		...
		for (var i = 1; i <= totalP; i++) {
			pStr +='<li><a href="javascript:void(0)" onclick="searchAllSong(15,'+i+')">'+i+'</a></li>';
		}
...
function paginationSong(totalP){
	...
	if(totalP>=5){
		pStr +='<li><a href="javascript:void(0)" onclick="paginatorPrevious('+totalP+')">&laquo;</a></li>';
		for (var j = 1; j <= 5; j++) {
			pStr +='<li><a href="javascript:void(0)" onclick="searchAllSinger(15,'+(j+(count*5))+')">'+(j+count*5)+'</a></li>';
		}
		pStr +='<li><a href="javascript:void(0)" onclick="paginatorNext('+totalP+')" >&raquo;</a></li>';
		...
		pStr +='<li><a href="javascript:void(0)" aria-label="Previous"><span aria-hidden="true">&laquo;</span></a></li>';
		for (var i = 1; i <= totalP; i++) {
			pStr +='<li><a href="javascript:void(0)" onclick="searchAllSinger(15,'+i+')">'+i+'</a></li>';
		}
		pStr +='<li><a href="javascript:void(0)" aria-label="Next"> <span aria-hidden="true">&raquo;</span></a></li>';
	}
	$(".pagination").html(pStr);
